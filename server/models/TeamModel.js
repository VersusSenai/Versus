...
class TeamModel {
  ...
  getAll = async (req) => {
    ...
    if(status.find((v)=>{v == "B"}) && req.user.role != "A"){
      throw new NotAllowedException("");
      
    }
    return await this.prisma.team.paginate({where: {status: {in: status}}}).withPages({
      page, limit 
    });
  };
  ...
  update = async (req) => {
    ...
    const team = await this.prisma.team.findFirst({ where: { id: parseInt(req.params.id) , status: "O"} });
 ...
 updateInvite = async(req)=>{
  ...
  isTeamOwner = async (user, teamId)=>{
    ...
    console.log(isTeamOwner != null || user.role== "A")
